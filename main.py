...
def create_tools(tools_locations):
    ...
    for record in tools_locations:
        for k, v in record.items():
            print('K', k)
            k = Tool(location_name=v['AdrName'], location_ip=v['AdrIP'], location_port=v['AdrPortNo'],
                     location_network=v['AdrNetworkNo'], location_network_plc=v['AdrNetworkPLCNo'],
                     location_id_line=v['AdrIDLine'], location_id_machine=v['AdrIDMachine'],
                     tool_addresses=v['AdrPLC'], conn=connstring)
            tools_list.append(k)
    return tools_list
    ...
    if trigger:
        try:
            ...
            if task == 1:
                ...
                reset_readable_to_plc = tool.generate_reset_plc_signals(task)
            ...
            #
            # tool.log_task_to_sql(task, tool_name, tool_data, operator_name)

        except Exception as e:
            log_error(e)
            response_to_plc = tool.generate_writable_plc_response(None, None, error=1)
            #tool.send_response_PLC(response_to_plc)
